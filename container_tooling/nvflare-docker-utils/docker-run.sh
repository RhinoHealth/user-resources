...
mkdir $tmpdir/poc
if ! docker run --rm -v "$tmpdir/unzip:/home/localuser/bin/unzip" -v "$tmpdir/poc:/home/localuser/poc" --network none "rhino-nvflare-localrun" /bin/sh -c 'echo y | PATH=/home/localuser/bin:$PATH poc -n '"$n_clients"' >/dev/null'; then
...
if [ $auto -eq 1 ]; then
  ...
  echo "To view server logs: docker logs rhino-nvflare-localrun-server"
  echo "To view client logs: docker logs rhino-nvflare-localrun-site-1"

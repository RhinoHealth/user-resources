...
class DataLoadSplitNN(object):
    def __init__(self, root, train=True, transform=None, returns="all", intersect_idx=None):
        """
        Args:
            root: data root
    ...
    def __build_subset__(self):
        # if intersect index provided, extract subset, otherwise use the whole set

        dataobj = self.unpickle(glob.glob(self.root + '/*')[0])
        data = dataobj['data'].reshape((len(dataobj['data']), 3, 32, 32))

...
class SplitNNConstants(object):
    ...
    TASK_SUBMIT_MODEL = "_splitnn_task_submit_model_"
...
class SplitNNController(Controller):
    def __init__(
        ...
        submit_model_task=SplitNNConstants.TASK_SUBMIT_MODEL,
        task_timeout: int = 10,
    ...
    def _submit(self, abort_signal: Signal, fl_ctx: FLContext):
        # self._check_targets(fl_ctx)
        # self.log_debug(fl_ctx, f"SplitNN training starting with {self.targets_names}.")
        #
        # # Create train_task
    ...
    def control_flow(self, abort_signal: Signal, fl_ctx: FLContext):
        try:
            ...
            # 3. Submit final model
            self._phase = AppConstants.TASK_SUBMIT_MODEL
            self.log_debug(fl_ctx, "Model submitted")
            self._submit(abort_signal=abort_signal, fl_ctx=fl_ctx)

...
import torch.cuda
from custom.network import PneumoniaModel  # ✅ fixed import
...
class PTModelLocator(ModelLocator):
    def __init__(self, exclude_vars=None, model=None, use_external_file=False):
    ...
    def locate_model(self, model_name, fl_ctx: FLContext) -> Union[DXO, None]:
        if model_name == PTConstants.PTServerName:
            try:
                ...
                self.log_error(fl_ctx, f"Error in retrieving {model_name}.", fire_event=False)
                return None
        else:

...
This folder contains various tools to assist in the local development, testing, and deployment to the FCP.

## Why Use This?

* Iterate quickly while developing locally.
* Emulate the conditions of the Rhino FCP, to help ensure your container images will work there.
...
Build a docker container image and push it to a container image repository. 

1. If you have not already done so, authenticate to your cloud provider CLI. For AWS, set up your ECR credentials with the AWS CLI. If you need assistance in setting these up, please follow the instructions in the [RhinoDocs](https://docs.rhinofcp.com/getting-started/quick-start-guide/pushing-containers-to-the-ecr) guide. For GCP, run `gcloud auth login` and follow the instructions.
2. Find your workgroup repository name in the container image registry (e.g. `rhino-gc-workgroup-my-workgroup`). If you need help finding your workgroup repository name, please follow this guide in [RhinoDocs](https://docs.rhinofcp.com/settings/viewing-and-adjusting-your-settings#viewing-container-and-artifact-credentials)
...
./path/to/utils/docker-push.sh rhino-gc-workgroup-my-workgroup tutorial-1-data-prep-v1
```

### [drive-admin-api.py](./drive-admin-api.py)
A utility Python script used to help facilitate the NVFlare FL network when using the `nvflare-docker-run.sh` script

### [gc-docker-run.sh](gc-docker-run.sh)
Build a docker container image and run it locally. 
...
chmod +x *.sh
cd path/to/your/code
./path/to/utils/gc-docker-run.sh path/to/input path/to/output
```
5. Debug and repeat until it works...


### [nvflare-docker-run-inference.sh](nvflare-docker-run-inference.sh)
Run inference with your code on a single dataset. 
...
chmod +x *.sh 
cd path/to/your/code
./path/to/utils/nvflare-docker-run-inference.sh path/to/input path/to/output path/to/weights
```

### [nvflare-docker-run.sh](nvflare-docker-run.sh)
Run FL training with your code using, locally, using Docker.  Recommended: `--auto`
...
chmod +x *.sh 
cd path/to/your/code
./path/to/utils/nvflare-docker-run.sh --auto path/to/input path/to/output
```
6. Debug and repeat until it works...

### [run-inference.sh](run-inference.sh)
A utility Shell script used to help facilitate the `nvflare-docker-run-inference.sh` script

### [upload-file-to-s3.sh](upload-file-to-s3.sh)
Upload a local directory to a workgroup's S3 bucket, for use with the Runtime External Storage feature for Code Runs
...
...
```
The same structure applies for the `/output` folder. 

If you are interested in saving data you **MUST** format the data in this structure.

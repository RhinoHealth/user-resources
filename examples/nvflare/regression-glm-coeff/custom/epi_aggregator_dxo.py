...
from epi_aggregation_helper import EPIAggregationHelper
...
class DXOAggregator(FLComponent):
    def __init__(
        ...
        self.aggregation_helper = EPIAggregationHelper(
    ...
    def accept(self, dxo: DXO, contributor_name, contribution_round, fl_ctx: FLContext) -> bool:
        ...
        if not self.aggregation_helper.verify_exog_names(data):
            self.log_error(fl_ctx, "exog_names mismatch")
            raise ValueError("exog_names mismatch")
        self.aggregation_helper.set_optimization_method(data)
        # aggregate

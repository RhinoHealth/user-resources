...
class PneumoniaTrainer(Executor):
    def __init__(
        ...
        self._train_dataset = torchvision.datasets.ImageFolder(train_dataset_folder, transform=transforms)
        self._test_dataset = torchvision.datasets.ImageFolder(test_dataset_folder, transform=transforms)
    ...
    def local_train(self, fl_ctx, model_global, abort_signal):
        # Basic training
        self.model.train()
        for epoch in range(self._epochs):
            ...
            for i, (batch_images, batch_labels) in enumerate(self._train_loader):
                ...
                if (i + 1) % 3000 == 0:
                    self.log_info(
                        fl_ctx, f"Epoch: {epoch}/{self._epochs}, Iteration: {i}, " f"Loss: {running_loss/images_count}"
                    )
            ...
            self.tb_writer.add_scalar("local_train_loss_per_epoch", running_loss / images_count, self.epoch_global)
    ...
    def local_valid(self, fl_ctx, abort_signal):
        ...
        with torch.no_grad():
            for i, (batch_images, batch_labels) in enumerate(self._test_loader):
                ...
                batch_labels = batch_labels.to(self.device)
                predictions = self.model(batch_images)
                total_loss += self.loss(predictions, batch_labels).cpu().detach().numpy()
                total_num_images += batch_images.size()[0]
        average_loss = total_loss / total_num_images
        current_round = fl_ctx.get_prop(AppConstants.CURRENT_ROUND)
        self.tb_writer.add_scalar("global_model_test_loss_per_round", average_loss, current_round)
        return average_loss
    ...
    def execute(self, task_name: str, shareable: Shareable, fl_ctx: FLContext, abort_signal: Signal) -> Shareable:
        try:
            if task_name == self._train_task_name:
                ...
                # Run training.
                self.local_train(fl_ctx, model_global, abort_signal)

...
class ConvNetTrainer(Executor):

    def __init__(self, lr=0.001, epochs=5, weight_decay=0.0001, train_task_name=AppConstants.TASK_TRAIN,
        ...
        # Training setup
        self.model = ConvNet(num_classes=4)
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        self.model.to(self.device)
        self.loss = nn.CrossEntropyLoss()
        self.optimizer = Adam(self.model.parameters(), lr=self._lr, weight_decay=self._weight_decay)
        ...
        self.train_count = len(glob.glob(train_path + '**/*.jpg'))
        # self.test_count = len(glob.glob(test_path + '**/*.jpg'))

        self.train_loader = DataLoader(
            torchvision.datasets.ImageFolder(root=train_path, transform=transforming_img),
            batch_size=64, shuffle=True
        )

        # self.test_loader = DataLoader(
        #     torchvision.datasets.ImageFolder(root=test_path, transform=transforming_img),
        #     batch_size=32, shuffle=True
        # )
        self._n_iterations = len(self.train_loader)
    ...
    def local_train(self, fl_ctx, weights, abort_signal):
        ...
        for epoch in range(self._epochs):
            ...
            train_accuracy = train_accuracy / self.train_count
            train_loss = train_loss / self.train_count

            # print('Epoch: ' + str(epoch) + ' Train Loss: ' + str(train_loss) + ' Train Accuracy: ' + str(train_accuracy))
            self.log_info(fl_ctx, f"Epoch: {epoch}/{self._epochs}, Iteration: {i}, "
                                  f"Loss: {train_loss / 3000}")
            ...
            #     _, prediction = torch.max(outputs.data_files, 1)
            #     test_accuracy += int(torch.sum(prediction == labels.data_files))
            #

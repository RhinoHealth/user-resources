...
class PTSecuredModelPersistor(ModelPersistor):
    ...
    def encrypt_weights(self, key, weights_path):
        # Read the model weights
        decrypted = Path(weights_path).read_bytes()
        
        # Import the public key
        public_key = RSA.import_key(key)
        cipher = PKCS1_OAEP.new(public_key)
        
        # Calculate chunk size (leaving room for padding)
        chunk_size = 190  # RSA 2048 can encrypt ~245 bytes, using 190 to be safe
        
        # Split data into chunks and encrypt each
        chunks = [decrypted[i:i + chunk_size] for i in range(0, len(decrypted), chunk_size)]
        encrypted_chunks = []
        ...
        Path(weights_path).write_bytes(encrypted)
    ...
    def save_model_file(self, save_path: str):
        ...
        if secret_run_params_file_path.is_file():
            with secret_run_params_file_path.open("rb") as secret_run_params_file:
                ...
                key = secret_run_params["encrypt_key"]
                self.encrypt_weights(key, save_path)

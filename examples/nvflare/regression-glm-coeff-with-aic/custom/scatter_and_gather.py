...
class ScatterAndGather(Controller):
    ...
    def control_flow(self, abort_signal: Signal, fl_ctx: FLContext) -> None:
        try:
            ...
            while self._current_round < self._start_round + self._num_rounds:
                ...
                # Finish training if got 'ABORT' from server
                incoming_dxo = from_shareable(data_shareable)
            ...
            if self._current_round == self._start_round + self._num_rounds:
                self.log_warning(fl_ctx, f"Reached max rounds - Regression did not reach the target accuracy within {self._num_rounds} iterations. Please interpret results with caution.")
    ...
    def _accept_train_result(self, client_name: str, result: Shareable, fl_ctx: FLContext) -> Optional[bool]:
        ...
        try:
            ...
            if str(e) == "exog_names mismatch":
                self.log_error(fl_ctx, "exog_names mismatch")
                self.system_panic(
                    f"exog_names mismatch between client {client_name} and previous clients - exiting at round {self._current_round}",
                    fl_ctx=fl_ctx,
                )
            return

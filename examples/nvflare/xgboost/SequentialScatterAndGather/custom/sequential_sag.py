...
class SequentialScatterAndGather(Controller):
    def __init__(
        self,
        min_clients: int = 1,
        num_rounds: int = 5,
        ...
        """The controller for SequentialScatterAndGather Workflow.

        The SequentialScatterAndGather workflow defines FederatedAveraging on all clients.
    ...
    def start_controller(self, fl_ctx: FLContext) -> None:
        self.log_info(fl_ctx, "Initializing SequentialScatterAndGather workflow.")
        self._phase = AppConstants.PHASE_INIT
    ...
    def control_flow(self, abort_signal: Signal, fl_ctx: FLContext) -> None:
        try:
            ...
            self.log_info(fl_ctx, "Beginning SequentialScatterAndGather training phase.")
            ...
            self.log_info(fl_ctx, "Finished SequentialScatterAndGather Training.")
        except BaseException as e:
            error_msg = f"Exception in SequentialScatterAndGather control_flow: {secure_format_exception(e)}"
        ...
        if rc and rc != ReturnCode.OK:
            if self.ignore_result_error:
                ...
                if rc in [ReturnCode.MISSING_PEER_CONTEXT, ReturnCode.BAD_PEER_CONTEXT]:
                    self.system_panic("Peer context is bad or missing. SequentialScatterAndGather exiting.", fl_ctx=fl_ctx)
                    return False
                elif rc in [ReturnCode.EXECUTION_EXCEPTION, ReturnCode.TASK_UNKNOWN]:
                    self.system_panic(
                        "Execution Exception in client training. SequentialScatterAndGather exiting.", fl_ctx=fl_ctx
                    )
                    return False
                elif rc in [
                    ...
                    self.system_panic("Execution result is not a shareable. SequentialScatterAndGather exiting.", fl_ctx=fl_ctx)
                    return False

# Copyright (c) 2023, Rhino Federated Computing
...
from splitnn_dataset import DataLoadSplitNN
...
from splitnn_workflow import SplitNNConstants, SplitNNDataKind
...
class LearnerSplitNN(Learner):
    def __init__(
        ...
        """Trainer for split learning.

        Args:
            dataset_root: directory with data.
    ...
    def initialize(self, parts: dict, fl_ctx: FLContext):
        ...
        self.train_dataset = DataLoadSplitNN(
        ...
        self.valid_dataset = DataLoadSplitNN(
            root=self.dataset_root,
            train=False,
    ...
    def save_local_model(self, fl_ctx: FLContext):
        model_path = os.path.join(self.app_root, "local_model.pt")
        ml = make_model_learnable(self.model.state_dict(), {})
        self.persistence_manager.update(ml)
        torch.save(self.persistence_manager.to_persistence_dict(), model_path)

    def submit_model(self, shareable: Shareable, fl_ctx: FLContext) -> Shareable:
        model_name = ModelName.FINAL_MODEL
        submit_model_result = self.get_final_model(model_name, fl_ctx)
    ...
    def get_final_model(self, model_name: str, fl_ctx: FLContext) -> Shareable:
        # Retrieve the final local model saved during training.
        self.local_model_file = os.path.join(self.app_root, "local_model.pt")
        if not os.path.isfile(self.local_model_file):
            return make_reply(ReturnCode.EXECUTION_RESULT_ERROR)
        if model_name == ModelName.FINAL_MODEL:
            try:
                # load model to cpu as server might or might not have a GPU
                model_data = torch.load(self.local_model_file, map_location="cpu")
            except BaseException as e:
                raise ValueError("Unable to load final model") from e
            ...
            if model_data:
                ...
                dxo = DXO(data_kind=SplitNNDataKind.WEIGHTS, data=model_weights)
                return dxo.to_shareable()
            else:
                # Set return code.
                self.log_error(fl_ctx, f"final local model not found at {self.local_model_file}.")
                return make_reply(ReturnCode.EXECUTION_RESULT_ERROR)
        else:

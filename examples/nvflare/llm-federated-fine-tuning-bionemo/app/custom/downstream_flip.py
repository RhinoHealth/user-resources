...
def main(cfg) -> None:
    ...
    src_dir = '/input'
    dst_dir = '/tmp'
    target_dir = os.path.join(dst_dir, os.path.basename(src_dir))
    shutil.copytree(src_dir, target_dir)

    train_dataset_file_paths = glob("/tmp/input/datasets/*/train/dataset.csv")
    val_dataset_file_paths = glob("/tmp/input/datasets/*/val/dataset.csv")
    test_dataset_file_paths = glob("/tmp/input/datasets/*/test/dataset.csv")

    cfg.model.data.dataset.train = [str(p) for p in train_dataset_file_paths][0]
    cfg.model.data.dataset.val = [str(p) for p in val_dataset_file_paths][0]
    cfg.model.data.dataset.test = [str(p) for p in test_dataset_file_paths][0]
